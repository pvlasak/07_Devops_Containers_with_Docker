...
- it is not necessary to create a network, docker compose will create default network where containers are running. 
...
### Docker Image in the private Nexus repository
**To access private Nexus repository** a user, role, blob store and remote repository itself must be created in Nexus UI. 
...
Before the first login to nexus repository, issuing of the login token must happen. In Nexus UI - Realms - `Docker Bearer Token` has to be active.
...
*{* <br>
 *"insecure-registries": ["ip_address:port"]* <br>
*}*

After the first login, the released token is added to file `~/.docker/config.json`
...
### Nexus as docker image
Starting **Nexus as docker container**, the volume can be created to persist the data. 
